...
imageset-config.yml: ## Generate templated imageset-config.yml (requires OCP_VERSION, e.g. 4.21.6 or 4.18.27)
	@echo "$(GREEN)Generating imageset-config.yml...$(NC)"
	@if [ -z "$(OCP_VERSION)" ]; then \
		echo "$(RED)✗ Error: OCP_VERSION variable is not set.$(NC)"; \
		echo "$(YELLOW)Usage: make imageset-config.yml OCP_VERSION=4.21.6$(NC)"; \
		echo "$(YELLOW)Or export OCP_VERSION before running: export OCP_VERSION=4.21.6 && make imageset-config.yml$(NC)"; \
		exit 1; \
	fi; \
